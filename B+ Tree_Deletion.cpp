...
{N->key[i-d]=O[i]->data;N->count++;}//putting all right values to new node, including median.

for(int i=d;i<2*d;i++)
{N->ptr[i-d]=O[i]->lchild;}
N->ptr[d-1]=O[2*d]->lchild;
N->ptr[d]=O[2*d]->rchild;//putting all right pointers to new node, including median's.
}
else
{for(int i=0;i<d;i++)
{T->key[i]=O[i]->data;}//putting all left values to old node.
...
BTPTR right=P->ptr[i+1];
median->lchild=node->ptr[node->count];
O[k++]=median;
...
else
{
    ...
    {if(node->ptr[i+1]!=NULL&&node->ptr[i+1]->count>d)
    {//This checks for successor to replace non leaf deletion node.
     cout<<"Replaced by successor!\n";
     int x=node->ptr[i+1]->key[0];
    delete_num(node->ptr[i+1]->key[0]);
    node->key[i]=x;
    }
    else if(node->ptr[i]!=NULL&&node->ptr[i]->count>d)
    {//This checks for predecessor to replace non leaf deletion node.
     cout<<"Replaced by predecessor!\n";
    int x=node->ptr[i]->key[node->ptr[i]->count-1];
    delete_num(node->ptr[i]->key[node->ptr[i]->count-1]);
    node->key[i]=x;
    }
    else
    {flag=0;
